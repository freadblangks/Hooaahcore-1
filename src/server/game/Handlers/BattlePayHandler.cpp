...
auto SendStartPurchaseResponse = [](WorldSession* session, Battlepay::Purchase const& purchase, Battlepay::PurchaseEligibility const& result, Battlepay::Error const& error = Battlepay::Error::None) -> void
{
...
auto MakePurchase = [](ObjectGuid targetCharacter, uint32 clientToken, uint32 productID, WorldSession* session) -> void
{
    ...
    if (!characterInfo)
    {
        SendStartPurchaseResponse(session, purchase, Battlepay::PurchaseEligibility::Ok, Battlepay::Error::PurchaseDenied);
        return;
    }
    ...
    {
        SendStartPurchaseResponse(session, purchase, Battlepay::PurchaseEligibility::Ok, Battlepay::Error::ItemUnavailable);
        return;
    }
    ...
    if (!accountBalance)
    {
        SendStartPurchaseResponse(session, *purchaseData, Battlepay::PurchaseEligibility::Ok, Battlepay::Error::InsufficientBalance);
        return;
    }
    ...
    if (!product.Items.empty())
    {
        if (product.Items.size() > GetBagsFreeSlots(player))
        {
            ...
            SendStartPurchaseResponse(session, *purchaseData, Battlepay::PurchaseEligibility::Ok, Battlepay::Error::PurchaseDenied);
            return;
        }
    }
    ...
    SendStartPurchaseResponse(session, *purchaseData, Battlepay::PurchaseEligibility::Ok);
    SendPurchaseUpdate(session, *purchaseData, Battlepay::PurchaseEligibility::Ok);
...
void WorldSession::HandleBattlePayConfirmPurchase(WorldPackets::BattlePay::ConfirmPurchaseResponse& packet)
{
    ...
    if (!product.Items.empty())
    {
        if (product.Items.size() > GetBagsFreeSlots(player))
        {
            ...
            player->SendCustomMessage(GetCustomMessage(Battlepay::CustomMessage::StoreBuyFailed), data);
            SendStartPurchaseResponse(this, *purchase, Battlepay::PurchaseEligibility::Ok, Battlepay::Error::ItemUnavailable);
            return;
        }
    }
    ...
    for (auto itr : product.Items)
    {
        if (GetBattlePayMgr()->AlreadyOwnProduct(itr.ItemID))
        {
            ...
            player->SendCustomMessage(GetCustomMessage(Battlepay::CustomMessage::StoreBuyFailed), data);
            SendStartPurchaseResponse(this, *purchase, Battlepay::PurchaseEligibility::Owned, Battlepay::Error::PurchaseDenied);
            return;
        }
    }
...
void WorldSession::HandleBattlePayPurchaseUnkResponse(WorldPackets::BattlePay::BattlePayPurchaseUnkResponse& /*packet*/)
{
    ...
    SendPurchaseUpdate(this, *purchaseData, Battlepay::PurchaseEligibility::Ok);
}
