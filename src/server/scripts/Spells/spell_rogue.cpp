        ...
        if(Unit* caster = GetCaster())
        {
            if(Unit* target = GetHitUnit())
            {
                ...
                if (roll_chance_i(chance))
                {
                    ...
                    SpellNonMeleeDamage dmg(caster, target, SPELL_ROGUE_SINISTER_STRIKE, { GetSpellInfo()->GetSpellXSpellVisualId(), 0 }, GetSpellInfo()->SchoolMask);
...
public:
    ...
    class spell_rog_weaponmaster_AuraScript : public AuraScript
    {
        ...
        bool CheckProc(ProcEventInfo& eventInfo)
        {
            ...
            SpellNonMeleeDamage damageLog(caster, target, triggerSpell->Id, { triggerSpell->GetSpellXSpellVisualId(), 0 }, triggerSpell->SchoolMask);
