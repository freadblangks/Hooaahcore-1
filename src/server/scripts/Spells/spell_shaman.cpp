...
enum ShamanSpells
{
    ...
    SPELL_SHAMAN_CRASHING_LIGHTNING_MOD_STORMSTRIKE         = 242286,
    SPELL_SHAMAN_CRASHING_LIGHTNING_DAMAGE                  = 195592, //triggered by SS, LL
    SPELL_FLAMETONQUE                                       = 193796,
...
private:
    ...
    void HandleHit(SpellEffIndex /*effIndex*/)
    {
        ...
        caster->ModifyPower(POWER_MAELSTROM, -20);
        
        targetsHit++;
        if (this->targetsHit == 2)
            caster->CastSpell(nullptr, SPELL_SHAMAN_CRASH_LIGTHNING_AURA, true);

         if (caster->HasAura(SPELL_SHAMAN_CRASHING_STORM_DUMMY))
         {
            caster->CastSpell(nullptr, SPELL_SHAMAN_CRASHING_STORM_AT, true);
            caster->CastSpell(nullptr, SPELL_SHAMAN_CRASHING_LIGHTNING_MOD_STORMSTRIKE, true);
         }
    }

    void Register() override
    {
        OnCheckCast += SpellCheckCastFn(spell_sha_crash_lightning::CheckCast);
        OnEffectHitTarget += SpellEffectFn(spell_sha_crash_lightning::HandleHit, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
    }
};
...
class spell_sha_stormstrike : public SpellScript
{
    ...
    void HandleOnHit()
    {
        ...
        if (!target)
            return;
        // SPELL_SHAMAN_CRASHING_LIGHTNING_DAMAGE not defined
        //if (GetCaster()->HasAura(SPELL_SHAMAN_CRASHING_STORM_DUMMY) && GetCaster()->HasAura(SPELL_SHAMAN_CRASH_LIGTHNING_AURA))
        //    GetCaster()->CastSpell(target, SPELL_SHAMAN_CRASHING_LIGHTNING_DAMAGE, true);
    }
...
public:
    ...
    void OnPeriodicProc()
    { 
        at->GetCaster()->CastSpell(at->GetPosition(), SPELL_SHAMAN_CRASHING_STORM_DAMAGE, true);
    }
};
