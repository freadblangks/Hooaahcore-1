...
struct npc_giant_boar : public ScriptedAI
{
    ...
    void OnSpellClick(Unit* clicker, bool& result) override
    {
        ...
        {
            // this first "if" will prevent players from cheating by using the huge pig again :) -Hooaah
...
struct npc_henry_garrick_156799 : public ScriptedAI
{
    ...
    void sQuestReward(Player* player, Quest const* quest, uint32 /*opt*/) override
    {
        ...
        {
            player->RemoveUnitFlag(UNIT_FLAG_NON_ATTACKABLE);
            //this scene is triggered from somewhere :D but where??? so dont use this part of c++ -Hooaah
...
struct npc_ralia_dreamchacer_156929 : public ScriptedAI
{
    ...
    void OnSpellClick(Unit* clicker, bool& result) override
    {
        if (Player* player = clicker->ToPlayer())
        {
            // this first "if" will prevent players from cheating by using ralia again :) -Hooaah
...
struct hrun_the_exiled_156900 : public ScriptedAI
{
    ...
    void JustDied(Unit* attacker) override
    {
        if (Player* player = attacker->ToPlayer())
        {
            ...
            {
                // ralia
                // this one is not triggering..... -Hooaah
