            ...
            if (Creature* creature = GetCaster()->ToCreature())
            {
                if (Unit* target = GetHitUnit())
                {
                    if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(SPELL_VOID_DIFFUSION_DMG))
                    {
                        ...
                        SpellNonMeleeDamage damageInfo(creature, target, SPELL_VOID_DIFFUSION_DMG, { spellInfo->GetSpellVisual(), 0 }, spellInfo->SchoolMask);
            ...
            if (Player* player = GetCaster()->ToPlayer())
            {
                if (Unit* target = GetHitUnit())
                {
                    if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(SPELL_DISRUPTING_SHADOWS_DMG))
                    {
                        ...
                        SpellNonMeleeDamage damageInfo(player, target, SPELL_DISRUPTING_SHADOWS_DMG, { spellInfo->GetSpellVisual(),0 }, spellInfo->SchoolMask);
                        damageInfo.damage = damage;
